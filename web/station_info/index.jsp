    	...
    	<fieldset>
		    ...
		    <table class="table table-hover">
              <thead>
                <tr>
					<th>station_id</th>
					<th>station_fullname</th>
					<th>station_shortname</th>
					<th>station_x</th>
					<th>station_y</th>
              ...
              <tbody>
              	<s:iterator var="item" value="#request.pager.datas">
                	<tr>
					<td>${station_id}</td>
					<td>${station_fullname}</td>
					<td>${station_shortname}</td>
					<td>${station_x}</td>
					<td>${station_y}</td>
            ...
            <s:if test="%{#request.pager.total > 0}">
					...
					<div id="pagination" class="pagination" >
						<ul>
							<pg:pager url="${contextPath}/front/station_info_index.action" items="${pager.total}"	
...
$(function(){
	$("#add-btn").click(function(){
		window.self.location = "${contextPath}/station_info/add.jsp";
	});
	$(".button-flat-highlight").click(function(){
		...
		$.dialog({
		    ...
		    ok: function () {
		    	/**确认信息*/
		    	$.post("${contextPath}/front/station_info_delete.action",{
					"station_info.tbid":tbid
				},function(msg){
					if(msg == 1){
						refresh();
					}
	...
	$(".button-flat-primary").click(function(){
		var tbid = $(this).attr("tbid");
		window.self.location = "${contextPath}/front/station_info_intoUpdate.action?station_info.tbid="+tbid;
	});
});
//刷新当前页面
function refresh(){
	window.self.location = "${contextPath}/front/station_info_index.action?pager.offset=${pager.offset}";
}
</script>
</body>
</html>