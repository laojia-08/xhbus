    	...
    	<fieldset>
		    ...
		    <table class="table table-hover">
              <thead>
                <tr>
					...
					<th>dirver_name</th>
					<th>dirver_sex</th>
					<th>dirver_birthday</th>
					<th>dirver_age</th>
					<th>dirver_telephone</th>
					<th>dirver_weChat</th>
                </tr>
              </thead>
              <tbody>
              	<s:iterator var="item" value="#request.pager.datas">
					...
					<td>${dirver_name}</td>
					<td>${dirver_sex}</td>
					<td>${dirver_birthday}</td>
					<td>${dirver_age}</td>
					<td>${dirver_telephone}</td>
					<td>${dirver_weChat}</td>
	                  	<td>
            ...
            <s:if test="%{#request.pager.total > 0}">
					...
					<div id="pagination" class="pagination" >
						<ul>
							<pg:pager url="${contextPath}/front/driver_info_index.action" items="${pager.total}"	
...
$(function(){
	$("#add-btn").click(function(){
		window.self.location = "${contextPath}/driver_info/add.jsp";
	});
	$(".button-flat-highlight").click(function(){
		...
		$.dialog({
		    ...
		    ok: function () {
		    	/**确认信息*/
		    	$.post("${contextPath}/front/driver_info_delete.action",{
					"driver_info.tbid":tbid
				},function(msg){
					if(msg == 1){
						refresh();
					}
	...
	$(".button-flat-primary").click(function(){
		var tbid = $(this).attr("tbid");
		window.self.location = "${contextPath}/front/driver_info_intoUpdate.action?driver_info.tbid="+tbid;
	});
});
//刷新当前页面
function refresh(){
	window.self.location = "${contextPath}/front/driver_info_index.action?pager.offset=${pager.offset}";
}
</script>
</body>
</html>