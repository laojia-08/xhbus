    	...
    	<fieldset>
		    ...
		    <table class="table table-hover">
              <thead>
                <tr>
					<th>attachment_id</th>
					<th>attachment_fullname</th>
					<th>attachment_address</th>
					<th>attachment_type</th>
					<th>attachment_moudle</th>
                </tr>
              </thead>
              <tbody>
              	<s:iterator var="item" value="#request.pager.datas">
                	<tr>
					<td>${attachment_id}</td>
					<td>${attachment_fullname}</td>
					<td>${attachment_address}</td>
					<td>${attachment_type}</td>
					<td>${attachment_moudle}</td>
	                  	<td>
            ...
            <s:if test="%{#request.pager.total > 0}">
					...
					<div id="pagination" class="pagination" >
						<ul>
							<pg:pager url="${contextPath}/front/attachment_info_index.action" items="${pager.total}"	
...
$(function(){
	$("#add-btn").click(function(){
		window.self.location = "${contextPath}/attachment_info/add.jsp";
	});
	$(".button-flat-highlight").click(function(){
		...
		$.dialog({
		    ...
		    ok: function () {
		    	/**确认信息*/
		    	$.post("${contextPath}/front/attachment_info_delete.action",{
					"attachment_info.tbid":tbid
				},function(msg){
					if(msg == 1){
						refresh();
					}
	...
	$(".button-flat-primary").click(function(){
		var tbid = $(this).attr("tbid");
		window.self.location = "${contextPath}/front/attachment_info_intoUpdate.action?attachment_info.tbid="+tbid;
	});
});
//刷新当前页面
function refresh(){
	window.self.location = "${contextPath}/front/attachment_info_index.action?pager.offset=${pager.offset}";
}
</script>
</body>
</html>